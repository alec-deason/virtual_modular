use crate::{
    delay_and_reverb::{BlockDelayLine, DelayLine},
    filter::{Biquad, DCBlocker, OnePole, Simper},
};
...
impl Node for BowedString {
    ...
    fn process(&mut self, input: Ports<Self::Input>) -> Ports<Self::Output> {
        ...
        for (i, r) in r.iter_mut().enumerate() {
            ...
            let nut = -self.nut_to_bow.current();
            let bridge = -self.string_filter.tick(self.bow_to_bridge.current()).tanh();
...
impl Node for PluckedString {
    ...
    fn process(&mut self, input: Ports<Self::Input>) -> Ports<Self::Output> {
        ...
        for (i, r) in r.iter_mut().enumerate() {
            ...
            let v = -self.string_filter.tick(self.line.current());
...
impl Node for Flute {
    ...
    fn process(&mut self, input: Ports<Self::Input>) -> Ports<Self::Output> {
        ...
        for (i, r) in r.iter_mut().enumerate() {
            ...
            let body_out = self.body.current();
            let embouchure_out = self.embouchure.current();
    ...
    fn process(&mut self, input: Ports<Self::Input>) -> Ports<Self::Output> {
        ...
        for (i, r) in r.iter_mut().enumerate() {
            ...
            if freq >= 1046.50 {
                ...
                body_cutoff /= 2.0;
                noise_scale *= 1.5;
                freq += 1.065_820_85e2 + 2.340_884_2e-1 * freq + -6.607_106e-5 * freq.powi(2);
            } else {
                freq += -3.642_305_1 + 3.345_885e-2 * freq;
            }
            if self.breath < 0.0 && note_trigger[i] > 0.5 && self.breathing == 0.0 {
                self.breathing = 1.0;
            }
            ...
            self.excitation_filter
                .set_parameters(excitation_cutoff, 0.0);
            let excitation = self.excitation_filter.low(noise);
            if self.breathing > 0.0 {
                *r += excitation;
                pressure = 0.0;
            ...
            let delay_out = if self.a_or_b {
                self.body_filter_b
                    .set_parameters(body_cutoff * self.old_freq as f32, 0.0);
                let mut r =
                    self.body_filter_b.low(self.delay_b.current() as f32) * self.crossfade as f32;
                self.body_filter_a
                    .set_parameters(body_cutoff * self.freq as f32, 0.0);
                r += self.body_filter_a.low(self.delay_a.current() as f32)
                    * (1.0 - self.crossfade as f32);
                r
            } else {
                self.body_filter_a
                    .set_parameters(body_cutoff * self.old_freq as f32, 0.0);
                let mut r =
                    self.body_filter_a.low(self.delay_a.current() as f32) * self.crossfade as f32;
                self.body_filter_b
                    .set_parameters(body_cutoff * self.freq as f32, 0.0);
                r += self.body_filter_b.low(self.delay_b.current() as f32)
                    * (1.0 - self.crossfade as f32);
                r
            };
...
impl ToneHole {
    fn tick(&mut self, pa_plus: f64, pb_minus: f64, r0: f64, hole_reflectivity: f64) -> (f64, f64) {
        ...
        self.line.tick(
            -(self
                .filter
                .low((pth_minus * -1.0 + pb_minus + middle + pa_plus) as f32) as f64
                * hole_reflectivity)
                .tanh(),
        );

        (middle + pa_plus, middle + pb_minus)
    }
...
impl Node for ToneHoleFlute {
    ...
    fn process(&mut self, input: Ports<Self::Input>) -> Ports<Self::Output> {
        ...
        for (i, r) in r.iter_mut().enumerate() {
            ...
            self.plus_lines_buffer
                .iter_mut()
                .zip(&self.plus_lines)
                .for_each(|(b, l)| *b = l.current());
            self.minus_lines_buffer
                .iter_mut()
                .zip(&self.minus_lines)
                .for_each(|(b, l)| *b = l.current());
            for (j, ((h, plus_in), minus_in)) in self
                .tone_holes
                .iter_mut()
                .zip(&self.plus_lines_buffer)
                .zip(&self.minus_lines_buffer)
                .enumerate()
            {
                let reflectivity = if hole_reflectivity > i as f64 {
                    0.0
                } else {
                    1.0
                };
                ...
                if j > 0 {
                    ...
                    self.plus_lines[0].tick(
                        -negative + thread_rng().gen_range(-1.0..1.0) * noise_scale[j] as f64,
                    );
                }
                if j < self.plus_lines.len() - 1 {
                    ...
                    self.minus_lines[j]
                        .tick(-(self.body_filter.low(positive as f32) as f64 * feedback).tanh());
                    *r = positive as f32;
                }
            }
        }
